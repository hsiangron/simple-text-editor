...
int Editor::cct_setconsoleborder(int set_cols, int set_lines, int set_buffer_cols, int set_buffer_lines)
{
	/* ??????????????????????? */
	COORD max_coord;
	max_coord = GetLargestConsoleWindowSize(hOutput); /* .X ?? .Y ??????????§Ü??§Ö????? */
	...
	/* ??????????????§Õ?§³(????§³????????????????) */
	COORD cr;
	cr.X = (set_buffer_cols == -1 || set_buffer_cols < set_cols) ? set_cols : set_buffer_cols;		//¦Ä????????????§³??set_cols????set_cols??¦Ä????????
	cr.Y = (set_buffer_lines == -1 || set_buffer_lines < set_lines) ? set_lines : set_buffer_lines;	//¦Ä????????????§³??set_lines????set_lines??¦Ä????????
